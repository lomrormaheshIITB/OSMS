...
class CreateProfileForm(forms.ModelForm):
	class Meta:
		...
		fields = ('firstname', 'lastname', 'personal_number', 'rank', 'ship_joining_date', 'section', 'remarks', 'access_level', 'department')

	def clean(self, *args, **kwargs):
		cleaned_data = super(CreateProfileForm, self).clean(*args, **kwargs)
...
class EditProfileForm(forms.ModelForm):
	...
	def clean(self, *args, **kwargs):
		cleaned_data = super(EditProfileForm, self).clean(*args, **kwargs)
		for key, val in zip(cleaned_data.keys(), cleaned_data.values()):
			try:
				cleaned_data[key] = val.upper()
			except AttributeError as e:

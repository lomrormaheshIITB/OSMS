...
let LoadTable = (init) => {
    ...
    for (var i = spares.length - 1; i >= 0; i--) {
        html += `
            <tr class = "table-row">
                <td>${url_view}${spares[i].pk}</td>
                <td>${spares[i].pattern_number}</td>              
                <td>${spares[i].equipment_class}</td>
                <td>${spares[i].description}</td>
                <td>${spares[i].category}</td>
                <td>${spares[i].denomination}</td>
                <td>${spares[i].quantity_authorised}</td>
                <td>${spares[i].quantity_available}</td>
                <td>${spares[i].critical}</td>
            </tr>
...
let DataTableCall = () => {
    $.when(
        // $.getScript('/static/assets/pages/scripts/table-datatables-fixedheader.js'),
        $.getScript('/static/data/js/datatable_searchlist.js'),

...
let LoadTable = (init) => {
    ...
    for (var i = spares.length - 1; i >= 0; i--) {
        html += `
        <tr class = "table-row">
        <td>${url_return}${spares[i].pk}/</td>
        <td>${spares[i].username}</td>
        <td>${spares[i].pattern_number}</td>
        <td>${spares[i].spare_class}</td>
        <td>${spares[i].equipment_class}</td>
        <td>${spares[i].description}</td>
        <td>${spares[i].quantity_toreturn}</td> 
        <td>${spares[i].quantity_held}</td>  
        <td>${spares[i].issue_remarks}</td>                 
    </tr>
        `;
    }
...
let DataTableCall = () => {
    $.when(
        $.getScript('/static/data/js/datatable_issuelist.js'),

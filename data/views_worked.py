...
# Remove the image from the specified spare
def RemoveImage(request, pk=None):
	if (pk):
		spare = Spares.objects.filter(pk=pk)
		if (spare.count() != 0):
			spare = spare[0]
			spare.image = 'default.png'
			spare.save()
			return HttpResponse('')
		...
		if (spare.count() != 0):
			...
			if (request.method == 'POST'):
				...
				if (form.is_valid()):
					...
					issue_entry = form.save(commit=False)
					issue_entry.spare = spare
					issue_entry.save()

					# Modify the remaining spare quantity
					spare.quantity_available -= quantity_issued
					spare.save()
					
					# spare_toreturn = IssueList.objects.filter(spare=spare)
					# if (spare_toreturn.count() == 0):
					# 	IssueList.objects.create(spare=spare, quantity_toreturn=quantity_issued, username= username)
					# else:
					# 	spare_toreturn = spare_toreturn[0]
					# 	spare_toreturn.quantity_toreturn += quantity_issued
					# 	spare_toreturn.save()
					
					# Modify the issue list table
					IssueList.objects.create(issue_entry=issue_entry, quantity_toreturn=quantity_issued)
					...
					if (spare.category == 'CONSUMABLE'):
						...
						if (demand_entry.count() == 0):
							...
							Demand.objects.create(spare=spare, quantity_todemand=quantity_issued, survey_entry=None)
						else:
...
def ReturnSpareView(request, pk=None):
	if (pk):
		...
		if (spare.count() != 0):
			spare = spare[0]
			issuelist_entry = IssueList.objects.filter(issue_entry.spare=spare)
			# Check if there is an issue entry
			if (issue_entry.count() == 0):
				messages.error(request, "Spare has not been issued!")
			else: 
				issuelist_entry = issuelist_entry[0]
				issue_entry = issuelist_entry.issue_entry
				username = issue_entry.username
			form = ReturnForm()
			...
			if (request.method == 'POST'):
				...
				if (form.is_valid()):					
					...
					issuelist_entry.quantity_toreturn -= quantity_returned
					if (issuelist_entry.quantity_toreturn <= 0):
						issuelist_entry.delete()
					else:
						issuelist_entry.save()
			...
			context = {
				...
				'spare': spare,
				'form': form,
				'quantity_toreturn': issue_entry.quantity_toreturn,
			...
			if (request.method == 'POST'):
				...
				if (form.is_valid()):
					...
					if (quantity_received == 0):
						...
						issue_entry = IssueList.objects.filter(spare=spare)
						if (issue_entry.count() != 0):
